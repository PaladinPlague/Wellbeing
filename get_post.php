...
if(isset($_GET["date"])) {
    ...
    $sql = "SELECT posts.title, posts.timestamp, posts.anonymous, users.username, posts.post_id FROM posts, users WHERE posts.user_id = users.user_id AND timestamp >= '$dateFrom' AND timestamp < '$dateTo' ORDER BY timestamp DESC;";
